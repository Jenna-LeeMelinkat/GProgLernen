package Aufgaben.ZweiKlassen.A_Personen;
 ...
 * 
 * Gegeben sei die Klasse `Aufgaben.ZweiKlassen.Personen.Person`. Eine `Aufgaben.ZweiKlassen.Personen.Person` hat
 * 
 * - einen Vornamen,
 * - und einen Nachnamen.
 * 
 * Wie `Aufgaben.ZweiKlassen.Personen.Person`en angelegt und ausgegeben werden können,
 * lässt sich den Beispielen in der `main()`-Methode entnehmen.
 * 
 * 1. Entwicklen Sie nun bitte eine Methode `countNames()`, die
 *    die Häufigkeit aller Vornamen in einer Liste von `Aufgaben.ZweiKlassen.Personen.Person`en
 *    bestimmt.
...
class Person____MapMostPopular {
    ...
    public static Map<String, Integer> countNames(List<Person> persons) {
        ...
        //-------------------------------------------------------------------------------
        // Sortieren der Aufgaben.Arrais.Map nach der Häufigkeit der Vornamen
        Map<String, Integer> sortierteMap = new LinkedHashMap<>();

        for (Map.Entry<String, Integer> eintrag : countedNames.entrySet()) {
            // Wenn die sortierte Aufgaben.Arrais.Map leer ist, füge den Eintrag einfach hinzu
            if (sortierteMap.isEmpty()) {
                sortierteMap.put(eintrag.getKey(), eintrag.getValue());
            } else {
                boolean eingefügt = false;

                // Durchlaufe die sortierte Aufgaben.Arrais.Map, um den richtigen Einfügepunkt zu finden
        ...
        /* Sortieren der Aufgaben.Arrais.Map nach der Häufigkeit der Vornamen
        Aufgaben.Arrais.Map<Aufgaben.String, Integer> sortierteMap = new LinkedHashMap<>();

        for (Aufgaben.Arrais.Map.Entry<Aufgaben.String, Integer> eintrag : countedNames.entrySet()) {
            boolean eingefügt = false;

            // Durchlaufen der sortierten Aufgaben.Arrais.Map, um den richtigen Einfügepunkt zu finden
            for (Aufgaben.Arrais.Map.Entry<Aufgaben.String, Integer> uff : sortierteMap.entrySet()) {
        ...
        countedNames.entrySet()
                // Verwende die Stream-API, um die Einträge der Aufgaben.Arrais.Map zu durchlaufen

                .stream()
                //wandelt um in Stream von Aufgaben.Arrais.Map.Entry-Objekten
                // repräsentiert einen Schlüssel-Wert-Paar in der Aufgaben.Arrais.Map.

                .sorted(Aufgaben.Arrais.Map.Entry.comparingByValue())
                //absteigende Reihenfolge: .sorted(Aufgaben.Arrais.Map.Entry.comparingByValue(Comparator.reverseOrder()))
