 ...
 * ## Shirts
 * 
 * Gegeben sei die Klasse `ZweiKlassen.Shirt`. Ein ZweiKlassen.Shirt hat
 * 
 * - eine Größe (XS, S, M, L, XL, etc.),
 * - und eine Farbe ("rot", "grün", "blau", "gelb", etc.).
 * 
 * Wie `ZweiKlassen.Shirt`s angelegt und ausgegeben werden können,
 * lässt sich den Beispielen in der `main()`-Methode entnehmen.
 * Es ist ferner eine Methode `factory()` gegeben, die eine Liste von
 * *n* zufälligen `ZweiKlassen.Shirt`s erzeugen kann.
 * 
 * 1. Ergänzen Sie die gegebene Klasse `ZweiKlassen.Shirt` gem. Java-Konventionen so,
 ...
 *    die es erlauben aus `ZweiKlassen.Shirt`-Objekten Farbe und Größe auszulesen.
 * 3. Entwickeln Sie ferner eine Methode `countColors()`,
 *    die zählt, wie viele `ZweiKlassen.Shirt`s einer Farbe in einer List von `ZweiKlassen.Shirt`s vorkommen.
 * 
...
class MainShirt____VorkommenNachFarben__Liste_Map_String_Integer {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(s1); // => "rotes XS-ZweiKlassen.Shirt"
        System.out.println(s2); // => "grünes L-ZweiKlassen.Shirt"
